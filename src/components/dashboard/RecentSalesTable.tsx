...
export function RecentSalesTable({ sales, isLoading }: RecentSalesTableProps) {
  if (isLoading) {
    return (
      <Card className="p-4 sm:p-5 md:p-6 border-border rounded-xl">
        <Skeleton className="h-4 w-28 sm:h-5 sm:w-32 mb-3 sm:mb-4" />
  ...
  return (
    <Card className="p-4 sm:p-5 md:p-6 border-border rounded-xl">
      <h3 className="mb-3 sm:mb-4 font-medium text-sm sm:text-base">Recent Sales</h3>
      <div className="space-y-2 sm:space-y-3">
        {sales.length === 0 ? (
          ...
          sales.slice(0, 5).map((sale, index) => (
            <motion.div
              key={sale.id}
              className="flex items-center justify-between gap-2 pb-2 sm:pb-3 border-b border-border last:border-0 last:pb-0"
