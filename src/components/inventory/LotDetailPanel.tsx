...
const gameColors: Record<string, string> = {
  ...
  riftbound:
    'bg-game-riftbound/20 text-game-riftbound border-game-riftbound/30',
};
    ...
    <Card>
      ...
      <CardContent className="px-4 pb-4 sm:px-6 sm:pb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          ...
          <DetailRow
            label="Qty on hand"
            value={`${lot.qty_on_hand} / ${lot.qty_initial}`}
          />
          <DetailRow
            label="Cost per card"
            value={formatCurrency(lot.cost_per_card)}
          />
          <DetailRow
            label="Total cost"
          ...
          <DetailRow
            label="Purchase date"
            value={formatDate(lot.purchase_date)}
          />
